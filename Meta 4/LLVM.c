...
void storeLLVM(ast_tree *what, ast_tree* where, int reg, int reg2){
    if(where!=NULL){
        if(strcmp(where->token->id, "Id")==0){
            ...
            printf("\tstore %s %%%d, %s* %%%d\n", typeLLVM(what->token->type), reg ,typeLLVM(what->token->type), reg2);
        }
        else{
            if(reg==-1){
                printf("\tstore %s %s, %s* @%s\n", typeLLVM(where->token->type), where->value,typeLLVM(what->token->type), what->value);
            }
            else{
...
void getFuncParams(ast_tree *node, int start){
    int count = start;
